...
int lanePos[4][4] = { {235, 117, 265, 117},
					 {265, 133, 235, 133},
					 {257, 110, 257, 140},
					 {243, 140, 243, 110} };			//lane positions, x, y of start of intersection, and x, y of end of intersection
...
void InitCars()
{
	...
	for (i = 0; i < NUM_CARS; i++)
	{
		...
		{
			carPos[i][0] = -10;
			carPos[i][1] = lanePos[0][1];
		}
		else if (carDir[i] == 1)
		{
			carPos[i][0] = 510;
			carPos[i][1] = lanePos[1][1];
		}
		else if (carDir[i] == 2)
		{
			carPos[i][1] = -10;
			carPos[i][0] = lanePos[2][0];
		}
		else if (carDir[i] == 3)
		{
			carPos[i][1] = 260;
			carPos[i][0] = lanePos[3][0];
		}
	}
}
...
void UpdatePositions()
{
	...
	if (!carGoalReached[carIndex])
	{
		if ((carDir[carIndex] == 0 && carStopPos[carIndex] >= lanePos[0][0])
			|| (carDir[carIndex] == 1 && carStopPos[carIndex] <= lanePos[1][0])
			|| (carDir[carIndex] == 2 && carStopPos[carIndex] >= lanePos[2][1])
			|| (carDir[carIndex] == 3 && carStopPos[carIndex] <= lanePos[3][1]))
		{
	...
	if (carVel[carIndex] <= 0 && carReachingGoal[carIndex])
	{
		...
		carSlope[carIndex] = (lanePos[newDir][3] - lanePos[carDir[carIndex]][1]) / ((float)(lanePos[newDir][2] - lanePos[carDir[carIndex]][0]));
	...
	if (carInIntersection[carIndex] && !carPassedInter[carIndex] && ((carNextDir[carIndex] == 0 && carPos[carIndex][0] > lanePos[0][2])
		|| (carNextDir[carIndex] == 1 && carPos[carIndex][0] < lanePos[1][2])
		|| (carNextDir[carIndex] == 2 && carPos[carIndex][1] > lanePos[2][3])
		|| (carNextDir[carIndex] == 3 && carPos[carIndex][1] < lanePos[3][3])))
	{
